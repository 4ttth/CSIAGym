...
def start_misc_server(challenge_id: int, user_id: int, file_path: str):
    """Returns (port, subdomain, expires_at, dynamic_flag)."""
    d = _post("/launch", body={
        "challenge_id": challenge_id,
        "user_id": user_id,
...
def stop_misc_server(challenge_id: int, user_id: int):
    _post("/stop", challenge_id=challenge_id, user_id=user_id)


def extend_misc_server(challenge_id: int, user_id: int):
    d = _post("/extend", challenge_id=challenge_id, user_id=user_id)
    return d["ok"], d["error"], d["expires_at"]


def misc_server_status(challenge_id: int, user_id: int) -> dict:
    return _get("/status", challenge_id=challenge_id, user_id=user_id)
