...
def _build_misc_container(
    challenge_id: int, user_id: int, file_path: str, port: int, flag: str
) -> str:
    """
    Misc challenge — either a .tar.gz archive (auto-detected server)
    or a single file (served statically via python3 -m http.server).
    """
    archive_path = os.path.realpath(file_path)
    work_dir = os.path.join(CHALLENGES_DIR, f"misc_{challenge_id}_u{user_id}")
    if os.path.exists(work_dir):
        shutil.rmtree(work_dir)
    ...
    if archive_path.endswith(".tar.gz") or archive_path.endswith(".tgz"):
        ...
        # Single file — copy into work_dir and serve statically
        os.makedirs(work_dir, exist_ok=True)
        dest = os.path.join(work_dir, os.path.basename(archive_path))
        shutil.copy2(archive_path, dest)
        image = "python:3.11-alpine"
        cmd_template = ["python", "-m", "http.server", "{port}"]
    ...
    container = _docker.containers.run(
        ...
        network_mode="bridge",
        read_only=False,
        name=f"chal_misc_{challenge_id}_u{user_id}",
        labels={
            "chal.type": "misc",
            "chal.challenge_id": str(challenge_id),
            "chal.user_id": str(user_id),
...
def launch(req: LaunchRequest):
    ...
    if ctype not in ("web", "nc", "misc"):
        raise HTTPException(400, "challenge_type must be 'web', 'nc', or 'misc'")
    ...
    try:
        if ctype == "web":
            ...
            container_id = _build_misc_container(
                req.challenge_id, req.user_id, req.archive_path, port, flag
            )
    except Exception as e:
        with _lock:
