...
def create_app(config_class=Config):
    ...
    with app.app_context():
        ...
        with db.engine.connect() as conn:
            ...
            # misc_challenges
            conn.execute(text(
                "CREATE TABLE IF NOT EXISTS misc_challenges "
                "(id INTEGER PRIMARY KEY, challenge_id INTEGER NOT NULL UNIQUE, "
            ...
            # challenge_submissions misc_file_path column
            sub_cols2 = {row[1] for row in conn.execute(text("PRAGMA table_info(challenge_submissions)"))}
            if 'misc_file_path' not in sub_cols2:
                conn.execute(text('ALTER TABLE challenge_submissions ADD COLUMN misc_file_path VARCHAR(500)'))
            conn.commit()
