...
from app.models import Challenge, UserChallengeSolve, User, ChallengeSubmission, FlagAttempt, WebChallenge, NcChallenge, MiscChallenge, DynamicFlag, ChallengeVote, BadgeRule, BadgeClaim, UserBadge, ChallengeBookmark, ChallengeSubscription, ChallengeOpen
...
def _record_correct_solve(challenge, user_id):
    ...
    if challenge.category in ('Web', 'Binary Exploitation', 'Misc'):
        try:
            ...
            elif challenge.category == 'Binary Exploitation':
                stop_nc_server(challenge.id, user_id)
            else:
                stop_misc_server(challenge.id, user_id)
        except Exception:
            pass
...
@challenges_bp.route('/challenges/<int:challenge_id>/launch-misc', methods=['POST'])
@login_required
@csrf.exempt
def launch_misc(challenge_id):
    challenge = Challenge.query.get_or_404(challenge_id)
    if challenge.category != 'Misc':
        return jsonify(ok=False, error='Not a misc challenge'), 400
    mc = MiscChallenge.query.filter_by(challenge_id=challenge_id).first()
    if not mc:
        return jsonify(ok=False, error='No file attached to this challenge.'), 404
    from app.challenge_runner import start_misc_server
    try:
        port, subdomain, expires_at, dynamic_flag = start_misc_server(challenge_id, current_user.id, mc.file_path)
        if dynamic_flag:
            _upsert_dynamic_flag(challenge_id, current_user.id, dynamic_flag)
        log_event(actor=current_user.username, action='instance_launch_misc', target=challenge.title, category='challenge')
        return jsonify(ok=True, port=port, url=f'http://{subdomain}:{port}/', expires_at=expires_at,
                       has_dynamic_flag=bool(dynamic_flag))
    except Exception as e:
        return jsonify(ok=False, error=str(e)), 500
...
@challenges_bp.route('/challenges/<int:challenge_id>/stop-misc', methods=['POST'])
@login_required
@csrf.exempt
def stop_misc(challenge_id):
    from app.challenge_runner import stop_misc_server
    stop_misc_server(challenge_id, current_user.id)
    return jsonify(ok=True)


@challenges_bp.route('/challenges/<int:challenge_id>/extend-misc', methods=['POST'])
@login_required
@csrf.exempt
def extend_misc(challenge_id):
    from app.challenge_runner import extend_misc_server
    ok, err, new_expires = extend_misc_server(challenge_id, current_user.id)
    if ok:
        return jsonify(ok=True, expires_at=new_expires)
    return jsonify(ok=False, error=err, expires_at=new_expires)


@challenges_bp.route('/challenges/<int:challenge_id>/misc-status')
@login_required
def misc_status(challenge_id):
    Challenge.query.get_or_404(challenge_id)
    from app.challenge_runner import misc_server_status
    st = misc_server_status(challenge_id, current_user.id)
    if st['running']:
        st['url'] = f"http://{st['subdomain']}:{st['port']}/"
        dyn = DynamicFlag.query.filter_by(challenge_id=challenge_id, user_id=current_user.id).first()
        st['has_dynamic_flag'] = bool(dyn)
