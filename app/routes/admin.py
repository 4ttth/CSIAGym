...
def approve_challenge(submission_id):
    ...
    if getattr(submission, 'misc_file_path', None):
...
def delete_challenge(challenge_id):
    ...
    if challenge.category == 'Binary Exploitation' and challenge.nc_challenge:
        try:
            ...
            pass
    if challenge.category == 'Misc' and challenge.misc_challenge:
        try:
            from app.challenge_runner import cleanup_challenge
            cleanup_challenge(challenge_id)
        except Exception:
    ...
    with db.engine.connect() as conn:
        ...
        conn.execute(text('DELETE FROM misc_challenges WHERE challenge_id=:c'), {'c': challenge_id})
