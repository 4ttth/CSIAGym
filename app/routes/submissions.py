...
MISC_CHALLENGES_DIR = os.path.join(
    os.path.dirname(__file__), '..', '..', 'instance', 'misc_challenges'
)
...
MAX_MISC_FILE_BYTES   = 100 * 1024 * 1024  # 100 MB cap for misc files
...
def new():
    ...
    if request.method == 'POST':
        ...
        misc_file = request.files.get('misc_file')
        misc_file_data = None
        if category == 'Misc' and misc_file and misc_file.filename:
            misc_file_data = misc_file.read()
            if len(misc_file_data) > MAX_MISC_FILE_BYTES:
                flash('Misc challenge file exceeds the 100 MB limit.', 'danger')
                return redirect(url_for('submissions.new'))
            misc_file.seek(0)
        elif category == 'Misc':
            flash('Miscellaneous challenges require a file or .tar.gz archive.', 'danger')
            return redirect(url_for('submissions.new'))
        ...
        if misc_file_data is not None:
            os.makedirs(MISC_CHALLENGES_DIR, exist_ok=True)
            real_misc_dir = os.path.realpath(MISC_CHALLENGES_DIR)
            ext = os.path.splitext(secure_filename(misc_file.filename))[1] or ''
            stored_name = uuid.uuid4().hex + ext
            misc_path = os.path.realpath(real_misc_dir + os.sep + stored_name)
            if not misc_path.startswith(real_misc_dir + os.sep):
                abort(400)
            with open(misc_path, 'wb') as out:
                out.write(misc_file_data)
            submission.misc_file_path = misc_path
        ...
        if current_user.is_admin:
            ...
            if submission.misc_file_path:
                from app.models import MiscChallenge
                db.session.add(MiscChallenge(challenge_id=challenge.id, file_path=submission.misc_file_path))
            db.session.commit()
            try:
