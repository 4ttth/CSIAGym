...
class ChallengeSubmission(db.Model):
    ...
    misc_file_path = db.Column(db.String(500), nullable=True)     # path to file/archive for Misc challenges
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class MiscChallenge(db.Model):
    """
    Stores the archive or single-file path for a Misc challenge.
    Served statically or via auto-detected server, same as WebChallenge.
    """
    __tablename__ = 'misc_challenges'

    id = db.Column(db.Integer, primary_key=True)
    challenge_id = db.Column(db.Integer, db.ForeignKey('challenges.id'), nullable=False, unique=True)
    file_path = db.Column(db.String(500), nullable=False)  # archive or single file
    host_port = db.Column(db.Integer, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    challenge = db.relationship('Challenge', backref=db.backref('misc_challenge', uselist=False))

    def __repr__(self):
        return f'<MiscChallenge challenge_id={self.challenge_id}>'
